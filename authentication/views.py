        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                if (len(password1) < 8):
                    ...
                    messages.error(
                        request, "password should should contain special characters ")
                    return render(request, 'authentication/register.html', context)
    ...
    def post(self, request):
        ...
        if username and password:
            ...
            if user:
                ...
                messages.error(
                    request, 'Your account is not active, please check your email')
                return render(request, 'authentication/login.html')
    ...
    def post(self, request):
        ...
        if user.exists():
            ...
            try:
                ...
                messages.success(
                    request, "we have sent a reset link to your email")
                return render(request, 'authentication/email_success.html')
            except Exception as error:
                print(error)

        messages.error(request, "something is Wrong!, reset link not sent")
...
class completePasswordReset(View):
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                messages.info(
                    request, "password reset  link is invalid, please request for a new one")
    ...
    def post(self, request, uidb64, token):
        ...
        if len(pass1) < 6:
            messages.error(
                request, "password too short! should be atleast 6 characters")
            return render(request, 'authentication/set-new-password.html', context)
        ...
        try:
            ...
            messages.success(
                request, "Password changed successfully, You can now login with your new password")
            return redirect('login')
        except Exception as e:
